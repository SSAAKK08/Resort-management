...
export default async function SignUpPage({ searchParams }) {
  ...
  return (
    <main className="grid min-h-screen bg-slate-50 dark:bg-slate-950 lg:grid-cols-2">
      <section
      ...
      >
        ...
        <div className="absolute inset-x-10 bottom-12 text-white">
          ...
          <p className="mt-3 max-w-xl text-xl">{t("signupDescription")}</p>
        </div>
      </section>
...
function ClerkSetupNotice({ title, body }) {
  return (
    <div
      role="alert"
      className="w-full max-w-md rounded-2xl border border-amber-300 bg-amber-50 p-6 text-amber-950 shadow-sm dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-100"
    >
      <h1 className="text-xl font-bold">{title}</h1>
      <p className="mt-3 leading-7">{body}</p>
      <div className="mt-4 space-y-2 rounded-xl bg-white/70 p-4 font-mono text-xs dark:bg-slate-950/50">
        <p>NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_...</p>
        <p>CLERK_SECRET_KEY=sk_...</p>
        <p>CLERK_STRAPI_SYNC_SECRET=...</p>
      </div>
    </div>
  );
}
