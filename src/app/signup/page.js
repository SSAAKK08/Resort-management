...
export default async function SignUpPage({ searchParams }) {
  ...
  return <main className="grid min-h-screen bg-slate-50 dark:bg-slate-950 lg:grid-cols-2"><section className="relative hidden bg-cover bg-center lg:block" style={{ backgroundImage: `url(${signInImage.src})` }}><div className="absolute inset-0 bg-slate-950/55" /><div className="absolute inset-x-10 bottom-12 text-white"><p className="font-bold uppercase tracking-widest text-cyan-200">{t('signupEyebrow')}</p><p className="mt-3 max-w-xl text-xl">{t('signupDescription')}</p></div></section><section className="relative flex items-center justify-center px-4 py-20"><div className="absolute right-5 top-5 flex gap-2"><LanguageSwitcher /><ThemeToggle /></div>{clerkConfigured ? <SignUp routing="hash" signInUrl="/login" fallbackRedirectUrl={nextPath} /> : <ClerkSetupNotice title={t('configurationTitle')} body={t('configurationBody')} />}</section></main>;
}
